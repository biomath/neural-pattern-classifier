...
def time_to_binary(spike_times, refractory_period, max_time=None):
    if max_time is None:
        max_time = max(spike_times)

    # Rounding spike times according to desired refractory period
    rp_decimal_accuracy = abs(Decimal(str(refractory_period)).as_tuple().exponent)
    spike_times = np.round(spike_times, rp_decimal_accuracy)

    time = np.arange(0, max_time, refractory_period)
...
def binary_to_time(raster, refractory_period):
    spike_indices, = np.where(raster == 1)
    spike_indices = spike_indices.astype(float)

    return spike_indices * refractory_period


def gaussian_moving_average_fft(spikes, refractory_period, sigma, dtype='times'):
    if dtype is 'times':
        binary_spikes = time_to_binary(spike_times=spikes, refractory_period=refractory_period)
...
def spike_timing_process_data(rasters_dict, refractory_period, sigma):
    gaussians_dict = dict((key, []) for key in rasters_dict.keys())
    for key in rasters_dict.keys():
        for sweep in rasters_dict[key]:
            gaussians_dict[key].append(gaussian_moving_average_fft(sweep, refractory_period, sigma, dtype='binary'))

    return gaussians_dict


def bin_train(train, refractory_period, bin_ms):
    binned_train = list()

    step_size = int(bin_ms * refractory_period * 1000)
...
def load_rasters(memory_name, refractory_period, pre_stimulus_time, post_stimulus_time,
    ...
    rasters_dict = generate_rasters_from_csv(spike_times, key_times, refractory_period,
                                                       pre_stimulus_time=pre_stimulus_time,
                                                       post_stimulus_time=post_stimulus_time)
    return rasters_dict


def generate_rasters_from_csv(spike_times, markers_df, refractory_period, pre_stimulus_time,
                                                 post_stimulus_time):
    binary_spikes = time_to_binary(spike_times, refractory_period)
    ...
    for i in np.arange(0, len(markers_df)):
        min_bound = int(np.round((markers_df.iloc[i, 0] - pre_stimulus_time) / refractory_period))
        max_bound = int(np.round((markers_df.iloc[i, 0] + post_stimulus_time) / refractory_period))

        # Fix for some rasters having imprecisions in the order of refractory_period
        ...
        if len(spikes_to_append) > ((post_stimulus_time - pre_stimulus_time) / refractory_period):
            while len(spikes_to_append) > ((post_stimulus_time - pre_stimulus_time) / refractory_period):
                spikes_to_append = np.delete(spikes_to_append, len(spikes_to_append) - 1)
        elif len(spikes_to_append) < ((post_stimulus_time - pre_stimulus_time) / refractory_period):
            while len(spikes_to_append) < ((post_stimulus_time - pre_stimulus_time) / refractory_period):
...
# def four_stim_load_rasters(memory_name, refractory_period, pre_stimulus_time, post_stimulus_time,
#                            key_name=None):
#     """
#     Function takes the recording name and the desired gaussian width (sigma) and returns
#     a dictionary (pre, ne, post) containing a subdictionary (stim49, stim50, stim51, stim52),
...
#         = four_stim_generate_rasters_from_csv(spike_times, key_times, refractory_period=refractory_period,
#                                               pre_stimulus_time=pre_stimulus_time,
#                                               post_stimulus_time=post_stimulus_time)
#
...
#
#
# def multi_load_rasters(recording, refractory_period, pre_stimulus_time, post_stimulus_time):
#     """
#     Function takes the recording name and the desired gaussian width (sigma) and returns
#     a dictionary (pre, ne, post) containing a subdictionary (stim49, stim50, stim51, stim52),
#     which contains a list of peristimulus spike rasters smoothed by a gaussian filter
...
#
#
# def four_stim_spike_timing_process_data(rasters_dict, refractory_period, sigma=None):
#     """
#     Function takes the recording name and the desired gaussian width (sigma) and returns
#     a dictionary (pre, ne, post) containing a subdictionary (stim49, stim50, stim51, stim52),
#     which contains a list of peristimulus spike rasters smoothed by a gaussian filter
...
#         pre_gaussian_stim49.append(gaussian_moving_average_fft(sweep, refractory_period, sigma, dtype='binary'))
#
#     pre_gaussian_stim50 = list()
#     for sweep in rasters_dict['stim50']:
#         pre_gaussian_stim50.append(gaussian_moving_average_fft(sweep, refractory_period, sigma, dtype='binary'))
#
#     pre_gaussian_stim51 = list()
#     for sweep in rasters_dict['stim51']:
#         pre_gaussian_stim51.append(gaussian_moving_average_fft(sweep, refractory_period, sigma, dtype='binary'))
#
#     pre_gaussian_stim52 = list()
#     for sweep in rasters_dict['stim52']:
#         pre_gaussian_stim52.append(gaussian_moving_average_fft(sweep, refractory_period, sigma, dtype='binary'))
#
...
#                                         post_stimulus_time, refractory_period):
#     binary_spikes = time_to_binary(spike_times, refractory_period)
#
#     spikes_stim49 = list()
#     spikes_stim50 = list()
...
#         min_bound = (key_times.iloc[i, 0] - pre_stimulus_time) / refractory_period
#         max_bound = (key_times.iloc[i, 0] + post_stimulus_time) / refractory_period
#
#         # Fix for some rasters having imprecisions in the order of refractory_period
#         # If more than max_bound-min_bound delete last data point (1ms usually)
#         # If less, add 0 to the end
...
# def ne_spike_timing_process_data(rasters_dict, refractory_period,
...
#         pre_gaussian_stim49.append(gaussian_moving_average_fft(sweep, refractory_period, pre_sigma, dtype='binary'))
#
#     pre_gaussian_stim50 = list()
#     for sweep in rasters_dict['pre']['stim50']:
#         pre_gaussian_stim50.append(gaussian_moving_average_fft(sweep, refractory_period, pre_sigma, dtype='binary'))
#
#     pre_gaussian_stim51 = list()
#     for sweep in rasters_dict['pre']['stim51']:
#         pre_gaussian_stim51.append(gaussian_moving_average_fft(sweep, refractory_period, pre_sigma, dtype='binary'))
#
#     pre_gaussian_stim52 = list()
#     for sweep in rasters_dict['pre']['stim52']:
#         pre_gaussian_stim52.append(gaussian_moving_average_fft(sweep, refractory_period, pre_sigma, dtype='binary'))
#
#     ne_gaussian_stim49 = list()
#     for sweep in rasters_dict['ne']['stim49']:
#         ne_gaussian_stim49.append(gaussian_moving_average_fft(sweep, refractory_period, ne_sigma, dtype='binary'))
#
#     ne_gaussian_stim50 = list()
#     for sweep in rasters_dict['ne']['stim50']:
#         ne_gaussian_stim50.append(gaussian_moving_average_fft(sweep, refractory_period, ne_sigma, dtype='binary'))
#
#     ne_gaussian_stim51 = list()
#     for sweep in rasters_dict['ne']['stim51']:
#         ne_gaussian_stim51.append(gaussian_moving_average_fft(sweep, refractory_period, ne_sigma, dtype='binary'))
#
#     ne_gaussian_stim52 = list()
#     for sweep in rasters_dict['ne']['stim52']:
#         ne_gaussian_stim52.append(gaussian_moving_average_fft(sweep, refractory_period, ne_sigma, dtype='binary'))
#
#     post_gaussian_stim49 = list()
#     for sweep in rasters_dict['post']['stim49']:
#         post_gaussian_stim49.append(gaussian_moving_average_fft(sweep, refractory_period, post_sigma, dtype='binary'))
#
#     post_gaussian_stim50 = list()
#     for sweep in rasters_dict['post']['stim50']:
#         post_gaussian_stim50.append(gaussian_moving_average_fft(sweep, refractory_period, post_sigma, dtype='binary'))
#
#     post_gaussian_stim51 = list()
#     for sweep in rasters_dict['post']['stim51']:
#         post_gaussian_stim51.append(gaussian_moving_average_fft(sweep, refractory_period, post_sigma, dtype='binary'))
#
#     post_gaussian_stim52 = list()
#     for sweep in rasters_dict['post']['stim52']:
#         post_gaussian_stim52.append(gaussian_moving_average_fft(sweep, refractory_period, post_sigma, dtype='binary'))
#
...
#
#
# def ne_load_rasters(recording, refractory_period, pre_stimulus_time, post_stimulus_time):
#     """
#     Function takes the recording name and the desired gaussian width (sigma) and returns
#     a dictionary (pre, ne, post) containing a subdictionary (stim49, stim50, stim51, stim52),
#     which contains a list of peristimulus spike rasters smoothed by a gaussian filter
...
#
#     pre_raster_stim49, pre_raster_stim50, pre_raster_stim51, pre_raster_stim52 \
#         = four_stim_generate_rasters_from_csv(pre_spike_times, pre_key_times, refractory_period=refractory_period,
#                                               pre_stimulus_time=pre_stimulus_time,
#                                               post_stimulus_time=post_stimulus_time)
#
#     ne_raster_stim49, ne_raster_stim50, ne_raster_stim51, ne_raster_stim52 \
#         = four_stim_generate_rasters_from_csv(ne_spike_times, ne_key_times, refractory_period=refractory_period,
#                                               pre_stimulus_time=pre_stimulus_time,
#                                               post_stimulus_time=post_stimulus_time)
#
#     post_raster_stim49, post_raster_stim50, post_raster_stim51, post_raster_stim52 \
#         = four_stim_generate_rasters_from_csv(post_spike_times, post_key_times, refractory_period=refractory_period,
#                                               pre_stimulus_time=pre_stimulus_time,
#                                               post_stimulus_time=post_stimulus_time)
#
...
# def ne_gonist_load_rasters(unit_name, refractory_period, pre_stimulus_time, post_stimulus_time, key_name=None):
#     if key_name is None:
#         key_name = unit_name
#
...
#
#
# def ne_gonist_spike_timing_process_data(rasters_dict, refractory_period, sigmas):
#     """
#     Function takes the recording name and the desired gaussian width (sigma) and returns
#     a dictionary (pre, ne, post) containing a subdictionary (stim49, stim50, stim51, stim52),
#     which contains a list of peristimulus spike rasters smoothed by a gaussian filter
...
#
#                 temp_gaussian_list.append(gaussian_moving_average_fft(train, refractory_period,
...
# def fad_ne_load_rasters(recording, refractory_period, pre_stimulus_time, post_stimulus_time):
#     """
#     Function takes the recording name and the desired gaussian width (sigma) and returns
#     a dictionary (pre, ne, post) containing a subdictionary (stim49, stim50, stim51, stim52),
#     which contains a list of peristimulus spike rasters smoothed by a gaussian filter
...
#         = four_stim_generate_rasters_from_csv(prefad_spike_times, prefad_key_times, refractory_period=refractory_period,
#                                               pre_stimulus_time=pre_stimulus_time,
#                                               post_stimulus_time=post_stimulus_time)
#
#     fadne_raster_stim49, fadne_raster_stim50, fadne_raster_stim51, fadne_raster_stim52 \
#         = four_stim_generate_rasters_from_csv(fadne_spike_times, fadne_key_times, refractory_period=refractory_period,
#                                               pre_stimulus_time=pre_stimulus_time,
#                                               post_stimulus_time=post_stimulus_time)
#
#     postfad_raster_stim49, postfad_raster_stim50, postfad_raster_stim51, postfad_raster_stim52 \
#         = four_stim_generate_rasters_from_csv(postfad_spike_times, postfad_key_times, refractory_period=refractory_period,
#                                               pre_stimulus_time=pre_stimulus_time,
#                                               post_stimulus_time=post_stimulus_time)
#
#     post_raster_stim49, post_raster_stim50, post_raster_stim51, post_raster_stim52 \
...
#
#
# def fad_ne_spike_timing_process_data(rasters_dict, refractory_period, sigma=None, pre_sigma=None, prefad_sigma=None,
#                                      fadne_sigma=None,
...
#         prefad_gaussian_stim49.append(gaussian_moving_average_fft(sweep, refractory_period, prefad_sigma, dtype='binary'))
#
#     prefad_gaussian_stim50 = list()
#     for sweep in rasters_dict['prefad']['stim50']:
#         prefad_gaussian_stim50.append(gaussian_moving_average_fft(sweep, refractory_period, prefad_sigma, dtype='binary'))
#
#     prefad_gaussian_stim51 = list()
#     for sweep in rasters_dict['prefad']['stim51']:
#         prefad_gaussian_stim51.append(gaussian_moving_average_fft(sweep, refractory_period, prefad_sigma, dtype='binary'))
#
#     prefad_gaussian_stim52 = list()
#     for sweep in rasters_dict['prefad']['stim52']:
#         prefad_gaussian_stim52.append(gaussian_moving_average_fft(sweep, refractory_period, prefad_sigma, dtype='binary'))
#
#     fadne_gaussian_stim49 = list()
#     for sweep in rasters_dict['fadne']['stim49']:
#         fadne_gaussian_stim49.append(gaussian_moving_average_fft(sweep, refractory_period, fadne_sigma, dtype='binary'))
#
#     fadne_gaussian_stim50 = list()
#     for sweep in rasters_dict['fadne']['stim50']:
#         fadne_gaussian_stim50.append(gaussian_moving_average_fft(sweep, refractory_period, fadne_sigma, dtype='binary'))
#
#     fadne_gaussian_stim51 = list()
#     for sweep in rasters_dict['fadne']['stim51']:
#         fadne_gaussian_stim51.append(gaussian_moving_average_fft(sweep, refractory_period, fadne_sigma, dtype='binary'))
#
#     fadne_gaussian_stim52 = list()
#     for sweep in rasters_dict['fadne']['stim52']:
#         fadne_gaussian_stim52.append(gaussian_moving_average_fft(sweep, refractory_period, fadne_sigma, dtype='binary'))
#
#     postfad_gaussian_stim49 = list()
#     for sweep in rasters_dict['postfad']['stim49']:
#         postfad_gaussian_stim49.append(gaussian_moving_average_fft(sweep, refractory_period, postfad_sigma, dtype='binary'))
#
#     postfad_gaussian_stim50 = list()
#     for sweep in rasters_dict['postfad']['stim50']:
#         postfad_gaussian_stim50.append(gaussian_moving_average_fft(sweep, refractory_period, postfad_sigma, dtype='binary'))
#
#     postfad_gaussian_stim51 = list()
#     for sweep in rasters_dict['postfad']['stim51']:
#         postfad_gaussian_stim51.append(gaussian_moving_average_fft(sweep, refractory_period, postfad_sigma, dtype='binary'))
#
#     postfad_gaussian_stim52 = list()
#     for sweep in rasters_dict['postfad']['stim52']:
#         postfad_gaussian_stim52.append(gaussian_moving_average_fft(sweep, refractory_period, postfad_sigma, dtype='binary'))
#
#     post_gaussian_stim49 = list()
#     for sweep in rasters_dict['post']['stim49']:
#         post_gaussian_stim49.append(gaussian_moving_average_fft(sweep, refractory_period, post_sigma, dtype='binary'))
...
#
#
# def six_stim_four_stim_generate_rasters_from_csv(spike_times, markers_df, refractory_period, pre_stimulus_time,
#                                                  post_stimulus_time):
#     binary_spikes = time_to_binary(spike_times, refractory_period)
#
#     spikes_stim49 = list()
...
#
#     for i in np.arange(0, len(markers_df)):
#         min_bound = (markers_df.iloc[i, 0] - pre_stimulus_time) / refractory_period
#         max_bound = (markers_df.iloc[i, 0] + post_stimulus_time) / refractory_period
#
#         # Fix for some rasters having imprecisions in the order of refractory_period
#         # If more than max_bound-min_bound delete last data point (1ms usually)
#         # If less, add 0 to the end
...
# def six_stim_spike_timing_process_data(rasters_dict, refractory_period, sigma):
#     """
#     Function takes the recording name and the desired gaussian width (sigma) and returns
#     a dictionary (pre, ne, post) containing a subdictionary (stim49, stim50, stim51, stim52),
#     which contains a list of peristimulus spike rasters smoothed by a gaussian filter
...
#     for sweep in rasters_dict['stim49']:
#         gaussian_stim49.append(gaussian_moving_average_fft(sweep, refractory_period, sigma, dtype='binary'))
#
#     gaussian_stim50 = list()
#     for sweep in rasters_dict['stim50']:
#         gaussian_stim50.append(gaussian_moving_average_fft(sweep, refractory_period, sigma, dtype='binary'))
#
#     gaussian_stim51 = list()
#     for sweep in rasters_dict['stim51']:
#         gaussian_stim51.append(gaussian_moving_average_fft(sweep, refractory_period, sigma, dtype='binary'))
#
#     gaussian_stim52 = list()
#     for sweep in rasters_dict['stim52']:
#         gaussian_stim52.append(gaussian_moving_average_fft(sweep, refractory_period, sigma, dtype='binary'))
#
#     gaussian_stim53 = list()
#     for sweep in rasters_dict['stim53']:
#         gaussian_stim53.append(gaussian_moving_average_fft(sweep, refractory_period, sigma, dtype='binary'))
#
#     gaussian_stim54 = list()
#     for sweep in rasters_dict['stim54']:
#         gaussian_stim54.append(gaussian_moving_average_fft(sweep, refractory_period, sigma, dtype='binary'))
#
...
#
#
# def six_stim_load_rasters(memory_name, refractory_period, pre_stimulus_time, post_stimulus_time,
...
#         = six_stim_four_stim_generate_rasters_from_csv(spike_times, key_times, refractory_period,
#                                                        pre_stimulus_time=pre_stimulus_time,
#                                                        post_stimulus_time=post_stimulus_time)
#
