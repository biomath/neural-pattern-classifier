...
class GUI(Tk):
    ...
    def check_memory_key_pairing(self):
        ...
        def show_problem_file(problem_memory_path, problem_key_path_or_list=None):
            ...
            if not isinstance(problem_key_path_or_list, list):
                cur_memory_name = "_".join(split("_*_", split(REGEX_SEP, problem_memory_path)[-1][:-4]))
                cur_key_name = "_".join(split("_*_", split(REGEX_SEP, problem_key_path_or_list)[-1][:-4]))
                ...
                for problem_key in problem_key_path_or_list:
                    cur_key_name = "_".join(split("_*_", split(REGEX_SEP, problem_key)[-1][:-4]))
                ...
                Label(memory_key_table_window, text=cur_memory_name).grid(row=0,
                                                                          column=0,
                                                                          sticky=E)
        memory_paths = glob(self.input_path.get() + sep + "*txt")

        memory_list = list()
        key_list = list()

        for memory_path in memory_paths:
            split_memory_path = split("_*_", split(REGEX_SEP, memory_path)[-1][:-4])
            cur_keyf = [item for idx, item in enumerate(split_memory_path) if
                        self.relevant_memory_name_pieces[idx] == 1]

            cur_key = glob(self.input_path.get() + sep + "*" + cur_keyf[0] + "*KEY*csv")
            ...
            if len(cur_key) > 0:
                ...
                cur_key = glob(self.input_path.get() + sep + "*" + "_*".join(cur_keyf) + "_*key*csv")  # try lower case
                if len(cur_key) > 1:
                    messagebox.showerror("Warning",
                                         "More than one key file was matched. Check your relevant memory name pieces!")
                    show_problem_file(memory_path, cur_key)
        ...
        for memory, key in zip(memory_list, key_list):
            cur_memory_name = "_".join(split("_*_", split(REGEX_SEP, memory)[-1][:-4]))
            cur_key_name = "_".join(split("_*_", split(REGEX_SEP, key)[-1][:-4]))
            Label(memory_key_table_window, text=cur_memory_name).grid(row=memory_key_table_window_row_counter,
                                                                      column=0,
                                                                      sticky=E)
            Label(memory_key_table_window, text=cur_key_name).grid(row=memory_key_table_window_row_counter,
    ...
    def show_memory_checkbar_window(self, temp_input_path, run_check_memory_key_pairings=False):
        ...
        split_name_options = glob(temp_input_path + sep + "*txt")[0]
        split_name_options = split("_*_", split(REGEX_SEP, split_name_options)[-1][:-4])
    ...
    def load_classifier_variables(self):
        ...
        # Clear old tempfiles if they exist
        self.clear_old_files()

        master_sheet_name = self.master_sheet_name_entry.get()
        accuracies_master_sheet_fullpath = self.output_path.get() + sep + master_sheet_name + "_accuracies.csv"
        statistics_master_sheet_fullpath = self.output_path.get() + sep + master_sheet_name + "_statistics.csv"
    ...
    def compile_result_csv(self):
        ...
        for cur_master_sheet_path in [accuracies_master_sheet_fullpath, statistics_master_sheet_fullpath]:
            process_tempfiles = glob(cur_master_sheet_path[:-4] + "*tempfile.csv")
            df_merged = (read_csv(f, sep=",", header=None) for f in process_tempfiles)
    ...
    def clear_old_files(self):
        # Temp process files that might exist from previous aborted/failed run
        process_tempfiles = glob(self.output_path.get() + sep + "*tempfile.csv")
        [remove(f) for f in process_tempfiles]
    ...
    def run_pattern_classifier(input_list):
        ...
        output_name = "_".join(split("_*_", split(REGEX_SEP, cur_memory_name)[-1][:-4])) + "_with_" + \
                      "_".join(split("_*_", split(REGEX_SEP, cur_key_name)[-1][:-4]))

        with PdfPages(cur_output_path + sep + output_name + ".pdf") as pdf:
            for method in classifier_methods:
                try:
                    run_classification(memory_name=cur_memory_name,
                                       ...
                                       accuracies_master_sheet_fullpath=accuracies_master_sheet_fullpath[:-4] + "_" + current_process().name + "_tempfile.csv",
                                       statistics_master_sheet_fullpath=statistics_master_sheet_fullpath[:-4] + current_process().name + "_tempfile.csv",
